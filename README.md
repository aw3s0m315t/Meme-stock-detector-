...
HOW USERS INTERACT: Entering a list of tickers to display information about, the user enters a size of list k, and the top k stocks and bottom k stocks by sentiment. Also amke it so we can see the stock price over a day, a week, a month, a year, and all time.
HOW RESULTS APPEAR: For each stock, we want a graph of the stock price over time. Also display a graph of the sentiment score of the stock over time, along with the prediction of whether it will go up or down or hover in the middle. Explain if its a good time to invest/sell.
VISUAL STYLE: Cozy cafe themed, lights are dim when the market is closed, but bright when the market is open. Also, display a cute sign at the top at an angle saying market open or market closed. make it like a czy cute cat cafe.
EXTRA FEATURES: Give a section where we can see a couple of the comments about the stock we are seeing to see how you made your decision.
